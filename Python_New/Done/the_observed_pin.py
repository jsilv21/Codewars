...
# that returns an array (or a list in Java and C#) of all variations for an observed PIN with a length of 1 to 8 digits. We could name the function getPINs (get_pins in python, GetPINs in C#). 
...
def get_pins(observed):
    ...
    #create list of observed number combos
    newlist = [num_adj[i] for i in observed]
    #getting cartesian product of unpacked list elements
    final = list(itertools.product(*newlist))
    #joining the list elements together into a string
    answer = [''.join(n) for n in final]
    return answer
